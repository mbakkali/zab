...
# La substitution d'une valeur par une référence a été retirée. Elle retirait le
# secret du disque, donc cassait tout ce qui lisait le fichier, et faisait du
# fournisseur distant la source de vérité — l'inverse du modèle voulu : le
# collecteur local fait foi, Secret Manager n'en est que l'image de secours.
# `_replace_dotenv_key` reste utilisé par `secrets_hub` pour mettre une valeur à
# jour en place, en préservant commentaire et disposition.
