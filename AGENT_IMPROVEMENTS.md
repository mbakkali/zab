...
- Second defect, found the same way — by verifying the real result rather than the plan: a project can hold several `.env` files (`project/.env` and `project/backend/.env`) carrying the same key with *different* values. The id stopped at the project, so the second write landed on the first as a new version: nothing was lost, version 1 still held it, but the mirror no longer said which file each value came from, and a restore would have taken the wrong one. The id now includes the sub-path within the project (`zab-<org>-<project>-<scope>-<key>`), with a matching `zab-scope` label. On a real workstation this turned 55 ids for 57 values into 57 for 57.
- Status: verified
...
- Trigger: mention
- Context: the change recorded below got the model backwards. It treated Secret Manager as the destination and rewrote each `.env` entry into an `sm://` reference, on the assumption that removing a secret from disk is an improvement. The operator's model is the opposite: `~/.config/zab/.env` is the collector every script reads, always plaintext, and the provider is a backup image for the day the keys are lost.
- Observation: the reference substitution was actively harmful, not merely misconceived. Any consumer of a rewritten file receives the literal string `sm://project/id` in place of an API key. It also made the remote copy load-bearing — losing access to the project, or working offline, would break applications that had been working from a local file a moment earlier. Two further gaps surfaced while correcting it. First, `collect` rewrote the hub as sorted `KEY=value` lines, deleting every comment and the file's ordering — the same class of loss as the trailing-comment bug below, one layer up, and inherited from `pm_env_sync`. Second, the mirror only covered the tracked catalogue: of 27 keys in a real hub, 9 were silently outside it and would never have been backed up, which defeats the entire point of a backup.
- Improvement: `push` became `mirror` — it reads the hub, writes to the provider, and touches no local file, with a test that asserts both the project `.env` and the hub are byte-identical afterwards. It reads the current remote version first and skips anything already equal, so repeated runs create no version churn. `pull` became `restore`, filling only what the hub is missing and never overwriting a present value. All writes now go through one `_upsert_dotenv` that updates keys in place and appends new ones, so comments, ordering and trailing notes survive. The mirror defaults to every non-empty key in the hub rather than the tracked catalogue — "tracked" decides what the dashboard shows, not what deserves to exist somewhere else. Added `secret_manager.map` for secrets that already exist under another name: without it the derived id (`zab-hermes-telegram-token`) misses the real one (`hermes-telegram-token`) and the mirror would create a duplicate beside it. Deleted `apply_secret_reference` and repointed the dashboard action at the mirror.
- Evidence: 535 passing, unchanged. On a real workspace: 27 of 27 hub values verified byte-identical to their mirrored copy, the hub unchanged by the operation, and the project `.env` restored to and confirmed identical with its pre-change state. The `zab-*` secrets created by the earlier, wrong behaviour were left in place — they hold correct values and are now what the mirror updates.
- Status: verified
- Lesson worth keeping: the first design was never asked for. "Manage my secrets" was read as "centralise them in a vault", which is the common shape, and the destructive half of that shape shipped before anyone said which direction authority ran in. A migration that moves data out of the place it is read from should state that plainly and get agreement before running, not after.
...
## 2026-09-20 - Weekly hygiene pass: manifest parity confirmed, workdir filter added to the conversation digest

- Trigger: mention
- Context: the weekly hygiene routine ran the offline parts of `docs/capability-audit.md` against a fresh checkout with no access to the operator's real Postgres, config, transcripts or launchd jobs — everything below was found and proven from the repository alone.
- Observation: `uv run pytest zab/tests -q` passed 566/566 on a clean checkout; `ruff check .` still reports only the same import-order/single-letter-variable style findings left alone by the 2026-08-09 pass (no new defect); `npx tsc --noEmit` and `./scripts/publish-check.sh` were both clean. A static parity check written for this pass — resolving every declared `cli` string against the real Typer command tree (subcommands and flags) and every declared `api` string against the real FastAPI route table — confirmed all 30 manifest capabilities resolve for real, including the two gaps this file's 2026-08-02 and 2026-08-09 entries already fixed (`tasks.sources_status` → `zab tasks sources --json`, `channels.list` → `zab channels list --json`); both are still pinned by `test_capabilities_manifest_parity.py` and `test_capabilities.py`. Two more manifest `cli` strings (`tasks.list`, `memory.search`) omit `--json` the way `channels.list` deliberately does — read as intentional given the existing pinned test on `channels.list`, not touched. Of the three frictions logged for the conversation digest on 2026-07-31, two were already fixed (2026-08-02: boilerplate-turn skip for `intent`; retained/shown/scanned counters); the third — no way to select conversations by working directory, so a session started in a sub-directory could be mislabelled by the digest's text-based project matching — was still open.
- Improvement: added `_extract_cwd()` to `agent_memory_import.py`, reading the session's own working directory from its transcript (Claude Code stamps `cwd` at the event's top level, Codex CLI nests it under the `session_meta` event's `payload`), and wired it into `collect_claude_documents`/`collect_codex_documents` as `metadata["cwd"]`. Added a `--workdir` option to `zab conversations digest` (and a `workdir` parameter to `build_conversation_digest`) that filters to sessions whose transcript-reported cwd matches exactly, after normalizing `~` and a trailing slash; the payload now reports `workdir` and `skipped_workdir` so a filtered run is legible rather than silently smaller. Providers whose transcript format never records a cwd are unaffected — the filter only ever excludes documents, never crashes on a missing field.
- Evidence: `uv run pytest zab/tests -q` — 570 passed (was 566), including a new `_extract_cwd()` unit test for both the top-level and nested-payload shapes, two end-to-end tests collecting real on-disk fixture transcripts and asserting `metadata["cwd"]` lands correctly for Claude and Codex, and a digest-level test asserting `--workdir` keeps only the matching session and reports the skipped count. `ruff check` clean on every touched file.
- Status: verified
- Open item (not fixed, needs a human call — restating what 2026-08-02 already flagged, since it is unchanged six weeks later): `zab/services/conversation_ledger/org_profiles.py`, `eval.py`, `channel_bindings.py`, `preflight.py` and several test fixtures still carry real, named client organizations, real contact names and real email addresses as hardcoded seed data and test data, in a repository confirmed public on GitHub. This predates the current commit and is not something that slipped in this week — the audit found no newly-introduced private data in the last 7 days of commits. It was left untouched again this pass: `org_profiles.py`'s hardcoded domains are load-bearing for the ledger's client-matching feature, so scrubbing them would change real behavior, not just remove a leak, and any history rewrite or repository-visibility change is a business decision outside a hygiene pass's authority. Flagged directly to the operator again rather than acted on.
