...
## 2026-08-23 - Weekly hygiene pass: a third manifest lie, a working-directory filter that was never added, and a real CI lock race this pass's own PR hit

- Trigger: mention
- Context: the weekly hygiene routine ran the offline parts of `docs/capability-audit.md` against a fresh checkout with no access to the operator's real Postgres, config or transcripts: full test suite, `ruff check .`, the UI typecheck, `npm run lint`, the publish guard, and a static parity check (this pass's own read plus an independent agent pass) of every capability's CLI form and API route against the real Typer tree and FastAPI routers.
- Observation: base health was clean — `uv run pytest zab/tests -q` passed 526/526 before any change this week (no commits landed between last week's hygiene merge and this run), `./scripts/publish-check.sh` stayed green, `npx tsc --noEmit -p tsconfig.app.json` was clean, and `ruff check .` showed the same 10 pre-existing style findings already triaged and left alone last week (import-order in two files, three single-letter loop variables in a test). `npm run lint` surfaced 44 ESLint errors, but all from two rules (`react-hooks/set-state-in-effect`, `react-refresh/only-export-components`) spread evenly across long-standing components, not a regression tied to any recent change — left alone as mass-cleanup risk outside this pass's scope, same call as ruff's findings. The two previously-known manifest gaps (`tasks.sources_status` declaring `zab config --json`; `channels.list` missing `--json`) stayed fixed. The static parity sweep found one the previous three passes hadn't: `system.check` declared `core="zab.services.system_check.run_system_check"` and `cli="zab doctor"`, but `zab doctor` never imports `zab.services.system_check` at all — it's an older, unrelated toolchain/config sanity check (SKILLS_ROOT, uv/node/npm, mempalace, memory DSN presence), not a flag away from the real payload the way the two earlier gaps were. Separately, the 2026-07-31 digest entry below had one friction still open: no way to select conversations by their actual working directory, only by fuzzy semantic project-name matching, so a session opened in an unregistered or misnamed directory had no precise way to be selected. Once this pass's own PR was open, its CI failed on a test this diff never touched: `test_mcps_sync_status_and_list` raised `sqlite3.OperationalError: database is locked` from `local_db.connect()`, even though three separate local full-suite runs (527/528 green each time) never reproduced it. Reproducing the lock directly found the real bug: `connect()` set `PRAGMA busy_timeout` *after* `PRAGMA journal_mode = WAL`, and worse, SQLite does not run the busy handler for that specific pragma at all — switching journal mode needs an exclusive lock and returns `SQLITE_BUSY` immediately regardless of `busy_timeout`, so only the very first connection(s) to ever open a fresh database file are exposed, exactly the kind of race concurrent CI processes hit and a single local run mostly won't.
- Improvement: corrected the `system.check` declaration to tell the truth (`cli=None`, `parity_notes` spelling out that MCP/API/UI already expose the real check and no CLI form does) rather than inventing a new CLI surface as a side effect of a parity sweep — a bigger call than this pass should make unilaterally. Added `--workdir` to `zab conversations digest`, filtering on the transcript's own recorded path/wing text (precise for Claude Code, which encodes the full cwd in its transcript folder name) instead of the project registry, with `skipped_workdir`/`workdir` surfaced in both the JSON payload and the markdown summary so an over-narrow filter reads as "0 kept, N filtered by directory" rather than a silently empty digest. For the lock race, moved the retry to the Python level instead of trusting SQLite's busy handler: `connect()` now retries a "database is locked" `OperationalError` up to 8 times with exponential backoff, and lets any other `OperationalError` through unchanged.
- Evidence: `uv run pytest zab/tests -q` passes 528/528 (526 baseline + 5 new tests: two pinning the `--workdir` filter keeping/dropping the right directory, one pinning that the manifest can't re-claim `zab doctor` as `system.check`'s CLI form, one holding a real write lock on a fresh database from a second connection and confirming `connect()` waits it out — shown to fail against the pre-fix `connect()` and pass against the fix). `ruff check .` and `./scripts/publish-check.sh` stayed exactly as before. No private paths, client names or credentials found in a diff review (no commits since last week's merge, so nothing new could have leaked).
- Status: verified
