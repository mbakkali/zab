...
- Trigger: debug
- Context: an operator asked why Zab was going unused. A read-only audit of 200 agent sessions found the MCP surface answered correctly but was almost never called, and concluded Zab duplicated cheaper paths. That conclusion was wrong, and the reason is worth recording: the audit had queried Postgres, where the ledger tables are empty, while every ledger write went to a local SQLite file.
- Observation: `postgres_store.TABLES` listed eighteen tables and none of the `ledger_*` ones, so `zab db status` silently under-reported the store by five tables holding thousands of interactions, hundreds of work packets, and the organisation registry. `import_legacy` had the same blind spot and never migrated them, which is why the Postgres side stayed empty after the migration was believed done. Two further consequences followed from the store being local: two machines running the same Zab each kept a ledger the other could not see, and `ledger.source_cursors` was a single shared key, so whichever machine synced last erased the other's read position — each then resumed a channel where the other had left it and skipped whatever arrived in between. Separately, `capabilities` declares `mcp: null` for exactly the three capabilities with no equivalent elsewhere (interactions, canonical work packets, context packet), while every capability that duplicates a registry, a tracker or a grep is exposed. The surface is the inverse of the value.
- Improvement: added `ledger_db`, a single door for the ledger. The SQL was already portable — `ON CONFLICT ... DO UPDATE SET excluded.*` is valid in both engines — so the adapter translates only what actually differed: parameter markers (`?` vs `%s`), positional row access where psycopg returns mappings, and `payload_json` being TEXT in one engine and `jsonb` in the other. All thirty-three call sites now go through it without knowing which engine answers. SQLite remains the fallback whenever no DSN resolves, matching what the rest of the store already does, so a machine without a database proxy writes locally instead of failing. Sync cursors, the append journal and the security inventory became per-machine; the shared data did not. A failing channel now records its reason and consecutive-failure count in its cursor, and a channel that answers without returning anything records `last_empty_at` — without those, a channel that stops leaves only a gap in the events table and no way to say when or why. New CLI: `zab ledger db`, `zab ledger import-sqlite [--apply]`, `zab ledger cursors [--machine]`; new API routes for both.
- Evidence: `uv run pytest zab/tests -q` at 529 passed / 1 skipped; eight new unit tests pin the adapter's three divergences and the fallback rule, with no database required. The legacy import ran idempotently on a real instance and moved every ledger row plus seven channel cursors; `zab db status` now counts the five tables it used to hide.
- Status: verified

## 2026-09-04 - Security status did not know which keys were supposed to exist

- Trigger: debug
- Context: the same audit found an agent concluding "no access" for a key that was present, and a connector reported as broken by Zab while the organisation's own checker reported it healthy.
- Observation: the list of tracked variables was a hand-written catalogue of forty-two names in `secrets_catalog.py`. Two keys that channel bindings themselves depend on were absent from it, so the status never looked at them. The catalogue also could not know about vaults declared outside Zab, so a key stored under a different name in an undeclared file read as missing — the exact divergence between Zab's connector check and the organisation's. And nothing was persisted: the status answered for the present moment on the machine asking, which cannot answer "does this key exist on the other machine" or "since when has it been gone".
- Improvement: `secrets_registry` reads an optional external, declarative registry whose path is configuration (`connectors_registry`). Zab stays generic: without that key nothing changes. The registry supplies the expected variable names, the connector each one serves — so a missing key names the service it breaks rather than just a variable — and the vault paths to scan, resolving relative paths against the repository that carries the registry rather than the working directory. `zab security scan` records a dated inventory per machine in the shared store: variable names, file paths, and the name actually encountered (an alias means searching for the canonical name finds nothing). No value is ever read or stored. `zab security machines` compares machines and separates a key missing here from a key missing everywhere.
- Evidence: six new tests covering the absent-registry case, multi-variable connectors, relative vault resolution and a corrupt registry file; on a real machine the tracked set grew from forty-two to sixty names, and a connector that had been reported degraded resolved correctly.
- Status: verified
- Open item (not fixed): `config.example.yaml` still carries two commented example channel identifiers naming a real organisation. Pre-existing, unrelated to this change, and left alone rather than folded into it.
