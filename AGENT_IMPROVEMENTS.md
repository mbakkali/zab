...
- Status: verified
- Fix commit: `5058fc7` (the 2026-08-09 hygiene pass restored the command and closed this item, but never flipped this entry's own status line)
...
## 2026-09-13 - Weekly hygiene pass: a stale status line, one real remaining digest friction, no new leaks

- Trigger: mention
- Context: the weekly hygiene routine ran the offline parts of `docs/capability-audit.md` against a fresh checkout, with no access to the operator's real Postgres, config or transcripts — everything below was found and proven from the repository alone.
- Observation: `uv run pytest zab/tests -q` (549 passed), `ruff check .` (10 pre-existing findings, all import-order/single-letter-variable style in files this pass did not touch), `cd zab-ui && npx tsc --noEmit -p tsconfig.app.json` (clean) and `./scripts/publish-check.sh` (green) all matched the state left by last week's pass; a diff of the last 7 days of commits turned up no new absolute paths, client names or credentials. Re-running a static parity check across all 30 declared capabilities' CLI forms (including every `--json` flag) and all 30 API routes found zero mismatches — the two gaps this file already knew about (`tasks.sources_status` declaring `zab config --json`, `channels.list` missing `--json`) were fixed in the 2026-08-02/08-09 passes but this file's own status lines were never flipped: the 2026-07-28 "restore `zab ws status --json`" entry still read `captured` although the command has worked (and been tested) since commit `5058fc7`, and the 2026-07-31 conversation-digest entry still read `captured` although two of its three listed frictions (boilerplate-skip in `intent`, the retained/shown counters) were already fixed. The third friction was real: `zab conversations digest` had no way to select conversations by their own working directory, only by the fuzzy semantic project/org matcher, so two sessions mentioning the same project keyword from different directories were indistinguishable. Separately, the previously flagged open item (a real client contact name and email address hardcoded in `zab/services/conversation_ledger/eval.py`'s fixture, alongside named client domains in `org_profiles.py`) is still present; it predates this week and remains a business decision (repo history already carries it) rather than a hygiene fix, per the 2026-08-02 entry's own reasoning.
- Improvement: added `--workdir <path>` to `zab conversations digest`, an exact filter on the transcript's own directory/path text, independent of the semantic project matcher, plus a `skipped_workdir_conversations` counter and a markdown line so a filtered-out conversation is never silently indistinguishable from "no match" (see the fix commit on this branch). Corrected the two stale `Status: captured` lines above to `verified` with the commits/passes that actually closed them, so the roadmap stops re-surfacing already-done work as debt.
- Evidence: new test `test_digest_workdir_filter_selects_exact_transcript_directory` in `zab/tests/test_conversation_digest.py` fails without the filter (both same-keyword conversations are shown) and passes with it (one shown, one counted in `skipped_workdir_conversations`); full suite green at 549 passed; static CLI/API parity check (`typer.main.get_command` walked against every declared `cli`, every declared `api` matched against `FastAPI.routes`) reports zero mismatches.
- Status: verified
- Open item (not fixed, needs a human call, not new): the real client contact email/name in `eval.py`'s fixture and the named client domains in `org_profiles.py`, first flagged 2026-08-02 — still present, still a business decision (repo visibility / history rewrite / client notification), not something a hygiene pass should silently rewrite.
