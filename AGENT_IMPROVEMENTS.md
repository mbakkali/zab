...
## 2026-09-06 - Weekly hygiene pass: manifest gaps already closed, a still-open remote VM debt item shipped

- Trigger: mention
- Context: the weekly hygiene routine ran the offline parts of `docs/capability-audit.md` against a fresh checkout — full test suite, `ruff check .`, the UI typecheck, the publish guard, a static parity check of every capability the manifest declares, and a scan for data that should not sit in a public repository.
- Observation: the two CLI parity gaps this routine's own brief names as "known and uncorrected" (`tasks.sources_status` pointing at a nonexistent `zab config --json`, and `zab channels list` missing `--json`) were already closed by the 2026-08-09 pass (`zab tasks sources --json`, `zab channels list --json`, manifest repointed) — verified against the live CLI (`--help` on both) rather than trusted from the brief. No new manifest parity gap turned up on the other 28 declared capabilities. `uv run pytest zab/tests -q` passed cleanly (524/524), `ruff check .` held at the same 10 known low-value style findings from the last pass, `npx tsc --noEmit` was clean, and `./scripts/publish-check.sh` stayed green. Of `AGENT_IMPROVEMENTS.md`'s three `Status: captured` entries, one (2026-07-28, `zab ws status --json`) was already fixed by the 2026-08-09 pass without its marker being updated; the conversation-digest entry (2026-07-31) had its boilerplate-skip and retained/shown-counter frictions already fixed, leaving only its working-directory filter open — implementing that safely would require decoding each provider's exact transcript-to-cwd convention, which this sandbox has no verified source for and no real transcript to check against, so it was left alone rather than shipping a guess. The remaining one (2026-08-01, "Audit remote VM readiness from Zab") described a real, reusable check that had never been turned into code: only a one-off manual audit had verified login-vs-non-login PATH parity and `$HOME` writability on a remote dev VM, so the same blind spot it found (a binary present in an interactive shell but silently absent from the non-interactive PATH an agent actually inherits over SSH) had no way to be caught again without repeating the manual audit. Confirmed still present, unrelated to this routine and already flagged in an earlier pass as a business decision rather than a hygiene fix: `zab/services/conversation_ledger/org_profiles.py` and `channel_bindings.py` still hardcode real named client organizations and the operator's real email addresses as built-in seed data, present since the repository's first commit — scrubbing the current files would not remove it from git history.
- Improvement: added `remote_vm.readiness_report()` plus `zab vm readiness --json` and `GET /api/remote-vm/readiness`, all read-only: it opens both a login (`ssh alias bash -lc`) and a non-login (`ssh alias cmd`) shell to compare their `$PATH`, checks `git`/`node`/`python3` are present in the non-login one specifically (the one an agent's own SSH command actually inherits), and confirms `$HOME` is writable without creating anything.
- Evidence: `uv run pytest zab/tests/test_remote_vm.py -q` (26 tests, 6 new: not-configured, ssh-absent, matching PATH all-ok, mismatched PATH with a missing non-login binary, unwritable `$HOME`, and the API route) proven to fail without the fix (asserting on `remote_vm.readiness_report` before it existed raises `AttributeError`) and pass with it; `uv run pytest zab/tests -q` passes 530/530; `ruff check` clean on every changed file; `./scripts/publish-check.sh` stayed green.
- Status: verified
